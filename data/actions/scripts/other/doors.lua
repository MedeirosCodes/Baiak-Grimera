function onUse(player, item, fromPosition, target, toPosition, isHotkey)
        ...
        if table.contains(questDoors, itemId) then
                if player:getStorageValue(item.actionid) ~= -1 then
                        ...
                        player:sendTextMessage(MESSAGE_INFO_DESCR, "The door seems to be sealed aga>
                end
                return true
        ...
        elseif table.contains(keys, itemId) then
         ...
         if table.contains(horizontalOpenDoors, itemId) or table.contains(verticalOpenDoors, itemId)>
                local doorCreature = Tile(toPosition):getTopCreature()
                if doorCreature then
                        toPosition.x = toPosition.x + 1
                        local query = Tile(toPosition):queryAdd(doorCreature, bit.bor(FLAG_IGNOREBL>
                        if query ~= RETURNVALUE_NOERROR then
                                toPosition.x = toPosition.x - 1
                                toPosition.y = toPosition.y + 1
                                query = Tile(toPosition):queryAdd(doorCreature, bit.bor(FLAG_IGNORE>
                        end

                        if query ~= RETURNVALUE_NOERROR then
                                player:sendTextMessage(MESSAGE_STATUS_SMALL, Game.getReturnMessage(>
                                return true
                        end
		...
		if item.actionid == 0 then
                        ...
                        player:sendTextMessage(MESSAGE_INFO_DESCR, "Está fechado.")
                end
                return true
        end
        return false
